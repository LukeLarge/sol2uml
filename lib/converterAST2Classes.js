 ...
 */
function convertAST2UmlClasses(
    node,
    relativePath,
    remappings,
    filesystem = false,
            ...
            if (childNode.type === 'ContractDefinition') {
                ...
                if (filesystem) {
                    ...
                    try {
                        ...
                        debug(
                            `Added filesystem import ${newImport.absolutePath} with class names ${newImport.classNames.map((i) => i.className)}`,
                        )
                    ...
                    const remappedFile = (0, exports.renameFile)(
                        childNode.path,
                        remappings,
                    )
                    const importPath =
...
function parseTypeName(typeName) {
    switch (typeName.type) {
        ...
        case 'ArrayTypeName':
            ...
            return [
                arrayElementType + '[' + length + ']',
                umlClass_1.AttributeType.Array,
            ]
        case 'Mapping':
            const key = typeName.keyType?.name || typeName.keyType?.namePath
