...
program
    ...
    .action(async (fileFolderAddress, options, command) => {
        try {
            ...
            parserGeneral_1.parserUmlClasses)(
                fileFolderAddress,
                combinedOptions,
            )
            if (
            ...
            let filteredUmlClasses = (0, filterClasses_1.filterHiddenClasses)(
                umlClasses,
                options,
            )
            // squash contracts
            if (options.squash) {
                filteredUmlClasses = (0, squashClasses_1.squashUmlClasses)(
                    filteredUmlClasses,
            ...
            if (options.baseContractNames || options.squash) {
                ...
                filteredUmlClasses = (0,
                filterClasses_1.classesConnectedToBaseContracts)(
                    filteredUmlClasses,
                    options.baseContractNames || [contractName],
                    options.depth,
                )
            ...
            const dotString = (0, converterClasses2Dot_1.convertUmlClasses2Dot)(
                filteredUmlClasses,
                combinedOptions.clusterFolders,
                combinedOptions,
            )
            // Convert Graphviz dot format to file formats. eg svg or png
            await (0, writerFiles_1.writeOutputFiles)(
                dotString,
...
WARNING: sol2uml does not use the Solidity compiler so may differ with solc. A known example is fixed-sized arrays declared with an expression will fail to be sized.\n`,
    ...
    .option(
        ...
        validators_1.validateSlotNames,
    )
    .option(
        '-st, --slotTypes <types>',
    ...
    .action(async (fileFolderAddress, options, command) => {
        try {
            ...
            if (
                !(0, regEx_1.isAddress)(fileFolderAddress) &&
                !combinedOptions.contract
            ) {
                throw Error(
            ...
            const storageSections = (0,
            converterClasses2Storage_1.convertClasses2StorageSections)(
                contractName,
                umlClasses,
                arrayItems,
            ...
            const optionVariables = (0,
            converterClasses2Storage_1.optionStorageVariables)(
                contractName,
                options.slotNames,
                options.slotTypes,
            )
            ...
            if (combinedOptions.data) {
                ...
                for (const storageSection of storageSections) {
                    ...
                    await (0, converterClasses2Storage_1.addDynamicVariables)(
                        storageSection,
                        storageSections,
                        combinedOptions.url,
                        storageAddress,
...
5. Contract dependencies are analysed so the files are merged in an order that will compile.\n`,
    ...
    .action(async (contractAddress, options, command) => {
        try {
            ...
            const etherscanParser = new parserEtherscan_1.EtherscanParser(
            ...
            await (0, writerFiles_1.writeSourceCode)(
                solidityCode,
                outputFilename,
            )
        } catch (err) {
...
The line numbers are from contract B. There are no line numbers for the red sections as they are not in contract B.\n`,
  ...
  0x1091588Cc431275F99DC5Df311fd8E1Ab81c89F3 will compare against the verified source code from Etherscan.`,
    ...
    .option(
        ...
        validators_1.validateLineBuffer,
    )
    .action(async (addressA, fileFoldersAddress, options, command) => {
        try {
            ...
            const aEtherscanParser = new parserEtherscan_1.EtherscanParser(
                combinedOptions.apiKey,
                combinedOptions.network,
                combinedOptions.explorerUrl,
            )
            if ((0, regEx_1.isAddress)(fileFoldersAddress)) {
                ...
                const bEtherscanParser = new parserEtherscan_1.EtherscanParser(
                ...
                // If flattening
                if (options.flatten) {
                    await (0, diffContracts_1.compareFlattenContracts)(
                        addressA,
                        addressB,
                        aEtherscanParser,
                        bEtherscanParser,
                    ...
                    await (0, diffContracts_1.compareVerifiedContracts)(
                        addressA,
                        aEtherscanParser,
                        addressB,
                        bEtherscanParser,
                ...
                await (0, diffContracts_1.compareVerified2Local)(
                    addressA,
                    aEtherscanParser,
                    localFolders,
                    combinedOptions,
