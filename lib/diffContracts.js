...
exports.displayFileDiffs =
    exports.displayFileDiffSummary =
    exports.diffVerifiedContracts =
    exports.diffVerified2Local =
    exports.compareFlattenContracts =
    exports.compareVerified2Local =
    exports.compareVerifiedContracts =
        void 0
...
const compareVerifiedContracts = async (
    addressA,
    aEtherscanParser,
    addressB,
    bEtherscanParser,
    ...
    const { contractNameA, contractNameB, files } = await (0,
    exports.diffVerifiedContracts)(
        addressA,
        addressB,
        aEtherscanParser,
    ...
    console.log(
        `to the ${bFileDesc}"${contractNameB}" contract with address ${addressB} on ${options.bNetwork || options.network}\n`,
    )
    ;(0, exports.displayFileDiffSummary)(files)
}
exports.compareVerifiedContracts = compareVerifiedContracts
const compareVerified2Local = async (
    addressA,
    aEtherscanParser,
    fileOrBaseFolders,
    options,
    ...
    const { contractNameA, files, local } = await (0,
    exports.diffVerified2Local)(addressA, aEtherscanParser, fileOrBaseFolders)
    if (!options.summary) {
        ;(0, exports.displayFileDiffs)(files, options)
    }
...
const compareFlattenContracts = async (
    addressA,
    addressB,
    aEtherscanParser,
    bEtherscanParser,
    ...
    if (options.bFile || options.aFile) {
        ...
        console.log(
            `to the "${options.bFile || options.aFile}" file for the "${contractNameB}" contract with address ${addressB} on ${options.bNetwork || options.network}\n`,
        )
    } else {
        console.log(
        ...
        console.log(
            `to the flattened "${contractNameB}" contract with address ${addressB} on ${options.bNetwork || options.network}\n`,
        )
    }
    return { contractNameA, contractNameB }
}
exports.compareFlattenContracts = compareFlattenContracts
const diffVerified2Local = async (
    addressA,
    etherscanParserA,
    fileOrBaseFolders,
    ...
    const bFiles = await (0, parserFiles_1.getSolidityFilesFromFolderOrFiles)(
        fileOrBaseFolders,
        ignoreFilesOrFolders,
    )
    ...
    for (const aFile of aFiles) {
        ...
        for (const baseFolder of fileOrBaseFolders) {
            bFile = bFiles.find((bFile) => {
                const resolvedPath = (0, path_1.resolve)(
                    process.cwd(),
                    baseFolder,
                    aFile.filename,
                )
...
const diffVerifiedContracts = async (
    addressA,
    addressB,
    etherscanParserA,
    etherscanParserB,
